...
public abstract class EntityLivingBase extends Entity {
   ...
   public static int maxHurtResistantTime;
   ...
   public boolean attackEntityFrom(DamageSource source, float amount) {
      if (this.isEntityInvulnerable(source)) {
         ...
         if (this.getHealth() <= 0.0F) {
            ...
            boolean flag = true;
            if ((float)this.hurtResistantTime > (float)maxHurtResistantTime / 2.0F) {
   ...
   protected void jump() {
      EventJump event = new EventJump((double)this.getJumpUpwardsMotion());
