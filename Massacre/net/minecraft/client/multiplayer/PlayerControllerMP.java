...
public class PlayerControllerMP {
   ...
   public float getBlockReachDistance() {
      ...
      return Massacre.INSTANCE.getModuleManager().getModule("Reach").getState() ? (float)MathUtil.getRandomInRange((Double)reach.minReach.getValue(), (Double)reach.maxReach.getValue()) : (this.currentGameType.isCreative() ? 5.0F : 4.5F);
   }
   ...
   public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn, C08PacketPlayerBlockPlacement packetPlayerBlockPlacement) {
      if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
         ...
         this.syncCurrentPlayItem();
         PacketUtil.sendPacketNoEvent(packetPlayerBlockPlacement);
