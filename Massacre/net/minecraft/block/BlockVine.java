         ...
         do {
            ...
            do {
               do {
                  ...
                  enumfacing = var5.next();
                  propertybool = getPropertyFor((EnumFacing)enumfacing);
               } while(!(Boolean)state.getValue(propertybool));
            } while(this.canPlaceOn(worldIn.getBlockState(pos.offset((EnumFacing)enumfacing)).getBlock()));
         ...
         if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos1)) {
            if (!flag) {
               ...
               while(true) {
                  ...
                  do {
                     ...
                     enumfacing3 = var23.next();
                  } while(!rand.nextBoolean() && this.canPlaceOn(worldIn.getBlockState(blockpos1.offset((EnumFacing)enumfacing3)).getBlock()));

                  iblockstate2 = iblockstate2.withProperty(getPropertyFor((EnumFacing)enumfacing3), false);
               }
            }
         } else {
            BlockPos blockpos3;
            if (enumfacing1.getAxis().isHorizontal() && !(Boolean)state.getValue(getPropertyFor(enumfacing1))) {
               ...
               if (block.blockMaterial == Material.air) {
                  ...
                  while(var14.hasNext()) {
                     enumfacing = var14.next();
                     if (rand.nextBoolean()) {
                        iblockstate3 = iblockstate3.withProperty(getPropertyFor((EnumFacing)enumfacing), false);
                     }
                  }
