...
public final class ClientUtils
{
    ...
    public static void sendMessage(final String message) {
        new ChatMessage.ChatMessageBuilder(true, true).appendText(message).setColor(EnumChatFormatting.WHITE).build().displayClientSided();
    }
    
    public static void sendMessage(final String message, final boolean prefix) {
        new ChatMessage.ChatMessageBuilder(prefix, true).appendText(message).setColor(EnumChatFormatting.WHITE).build().displayClientSided();
    }
    ...
    public static void sendPacketInQueue(final Packet p) {
        Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(p);
    }
    
    public static void sendPacketSkipQueue(final Packet p) {
        Minecraft.getMinecraft().getNetHandler().getNetworkManager().dispatchPacket(p, null);
    }
    
    public static void updatePosition(final double x, final double y, final double z) {
        packet(new C03PacketPlayer.C04PacketPlayerPosition(x, y, z, Minecraft.getMinecraft().thePlayer.onGround));
    }
