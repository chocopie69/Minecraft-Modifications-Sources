...
public class ItemPhysics extends Module {
    ...
    public static MathUtils random;
    
    public ItemPhysics() {
        super("ItemPhysics", "ItemPhysics", 0 , Category.VISUALS);
    }
    ...
    public static void doRender(final RenderEntityItem renderer, final Entity entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        ...
        for (int k = 0; k < j; ++k) {
            if (flag2) {
                GlStateManager.pushMatrix();
                if (k > 0) {
                    final float f4 = (random.random.nextFloat() * 2.0f - 1.0f) * 0.15f;
                    final float f5 = (random.random.nextFloat() * 2.0f - 1.0f) * 0.15f;
                    final float f6 = (random.random.nextFloat() * 2.0f - 1.0f) * 0.15f;
