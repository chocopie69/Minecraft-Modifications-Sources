...
import summer.cheat.eventsystem.events.player.EventPostMotionUpdate;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdateWalkingPlayer() {
        EventUpdate EventUpdatePre = new EventUpdate(this.posX, this.posY, this.posZ, this.onGround, this.rotationYaw,
                this.rotationPitch, this.lastReportedYaw, this.lastReportedPitch, Stage.PRE);
        EventUpdatePre.call();
        boolean flag = isSprinting();
        this.serverSidePosX = EventUpdatePre.getX();
        this.serverSidePosY = EventUpdatePre.getY();
        this.serverSidePosZ = EventUpdatePre.getZ();
        ...
        if (this.mc.gameSettings.thirdPersonView != 0) {
            this.renderYawOffset = EventUpdatePre.getYaw();
            this.serverSidePitch = EventUpdatePre.getPitch();
            this.rotationYawHead = EventUpdatePre.getYaw();
        }
        if (isCurrentViewEntity()) {
            double d0 = EventUpdatePre.getX() - this.lastReportedPosX;
            double d1 = EventUpdatePre.getY() - this.lastReportedPosY;
            double d2 = EventUpdatePre.getZ() - this.lastReportedPosZ;
            double d3 = (EventUpdatePre.getYaw() - this.lastReportedYaw);
            double d4 = (EventUpdatePre.getPitch() - this.lastReportedPitch);
            boolean flag2 = !(d0 * d0 + d1 * d1 + d2 * d2 <= 9.0E-4D && this.positionUpdateTicks < 20);
            boolean flag3 = !(d3 == 0.0D && d4 == 0.0D);

            if (this.ridingEntity == null) {
                if (flag2 && flag3) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(EventUpdatePre.getX(),
                            EventUpdatePre.getY(), EventUpdatePre.getZ(), EventUpdatePre.getYaw(),
                            EventUpdatePre.getPitch(), EventUpdatePre.isOnGround()));
                } else if (flag2) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(EventUpdatePre.getX(),
                            EventUpdatePre.getY(), EventUpdatePre.getZ(), EventUpdatePre.isOnGround()));
                } else if (flag3) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(EventUpdatePre.getYaw(),
                            EventUpdatePre.getPitch(), EventUpdatePre.isOnGround()));
                } else {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer(EventUpdatePre.isOnGround()));
                }
            } else {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
                        this.motionZ, this.rotationYaw, this.rotationPitch, EventUpdatePre.isOnGround()));
                flag2 = false;
            }
            ...
            if (flag2) {
                this.lastReportedPosX = EventUpdatePre.getX();
                this.lastReportedPosY = EventUpdatePre.getY();
                this.lastReportedPosZ = EventUpdatePre.getZ();
                this.positionUpdateTicks = 0;
            }

            if (flag3) {
                this.lastReportedYaw = EventUpdatePre.getYaw();
                EventUpdatePre.setLastYaw(this.lastReportedYaw);
                this.lastReportedPitch = EventUpdatePre.getPitch();
                EventUpdatePre.setLastPitch(this.lastReportedPitch);
            }
        }
        EventUpdate EventUpdatePost = new EventUpdate(this.posX, this.posY, this.posZ, this.onGround, this.rotationYaw,
                this.rotationPitch, this.lastReportedYaw, this.lastReportedPitch, Stage.POST);
        EventUpdatePost.call();

        EventUpdate.setLastX(EventUpdatePre.getX());
        EventUpdate.setLastY(EventUpdatePre.getY());
        EventUpdate.setLastZ(EventUpdatePre.getZ());
    }
    ...
    public boolean isPushedByWater() {
        EventWater ew = new EventWater(true);
        ew.call();
        return !(CheatManager.getInstance(NoSlowDown.class).isToggled() && NoSlowDown.noWater.getValBoolean());
    }
    ...
    public void sendChatMessage(String message) {
        if (!Summer.INSTANCE.commands.processCommand(message))
    ...
    public void moveEntity(final double x, final double y, final double z) {
        EventMotion e = new EventMotion(x, y, z);
        e.call();
    ...
    public void onLivingUpdate() {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            EventSlowDown esd = new EventSlowDown(0.2F, 0.2F);
            esd.call();
            if (!esd.isCancelled()) {
                this.movementInput.moveStrafe *= esd.getStrafe();
                this.movementInput.moveForward *= esd.getForward();
                if (esd.getStrafe() == 0.2F && esd.getForward() == 0.2F) {
    ...
    public void setMoveSpeedAris(EventMotion event, double speed) {
