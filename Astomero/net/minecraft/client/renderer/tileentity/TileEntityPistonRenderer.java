    ...
    public void renderTileEntityAt(final TileEntityPiston te, final double x, final double y, final double z, final float partialTicks, final int destroyStage) {
        ...
        if (block.getMaterial() != Material.air && te.getProgress(partialTicks) < 1.0f) {
            ...
            if (block == Blocks.piston_head && te.getProgress(partialTicks) < 0.5f) {
                ...
                final BlockPistonExtension.EnumPistonType blockpistonextensionenumpistontype = (block == Blocks.sticky_piston) ? BlockPistonExtension.EnumPistonType.STICKY : BlockPistonExtension.EnumPistonType.DEFAULT;
                IBlockState iblockstate2 = Blocks.piston_head.getDefaultState().withProperty(BlockPistonExtension.TYPE, blockpistonextensionenumpistontype).withProperty((IProperty<Comparable>)BlockPistonExtension.FACING, (Comparable)iblockstate.getValue((IProperty<V>)BlockPistonBase.FACING));
