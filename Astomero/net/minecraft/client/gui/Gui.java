...
public class Gui
{
    ...
    public static void drawModalRectWithCustomSizedTexture2(final float x, final float y, final float u, final float v, final float width, final float height, final float textureWidth, final float textureHeight) {
        final float f = 1.0f / textureWidth;
        final float f2 = 1.0f / textureHeight;
        final Tessellator tessellator = Tessellator.getInstance();
        final WorldRenderer worldrenderer = tessellator.getWorldRenderer();
    ...
    public static void drawScaledCustomSizeModalRect(final double x, final double y, final float u, final float v, final double uWidth, final double vHeight, final double width, final double height, final float tileWidth, final float tileHeight) {
        ...
        var13.pos(x, y + height, 0.0).tex(u * var10, (v + (float)vHeight) * var11).endVertex();
        var13.pos(x + width, y + height, 0.0).tex((u + (float)uWidth) * var10, (v + (float)vHeight) * var11).endVertex();
        var13.pos(x + width, y, 0.0).tex((u + (float)uWidth) * var10, v * var11).endVertex();
        var13.pos(x, y, 0.0).tex(u * var10, v * var11).endVertex();
        var12.draw();
    }
