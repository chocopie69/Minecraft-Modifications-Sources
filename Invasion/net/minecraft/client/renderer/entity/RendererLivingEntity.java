...
import Velo.impl.Modules.visuals.Chams;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                if(entity != Minecraft.getMinecraft().thePlayer) {
                	...
                	if(CustomVelo.isEnabled) {
                		if(Killaura.isEnabled) {
                			f8 = CustomVelo.pitch + (CustomVelo.pitch - CustomVelo.lastTickPitch) * partialTicks;
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_)
    {
    	...
    	String theme = ESP2D.theme.getMode();
    	String mode = ESP2D.mode.getMode();
      	float rainbowSpeed = (float) ESP2D.rainbowSpeed.getValue();
    	float blue = (float) ESP2D.blue.getValue();
    	float green = (float) ESP2D.green.getValue();
    	float red = (float) ESP2D.red.getValue();
    	
    	Color cfo = null;

        if (theme.equals("Custom Color")) {
        	cfo = new Color((int) red, (int) green, (int) blue);
        }

        if (theme.equals("Chill Rainbow")) {
        	cfo = ColorManager.rainbow(100, (int) rainbowSpeed, 0.5f, 1, 0.8f);
        }

        if (theme.equals("Rainbow")) {
        	cfo = ColorManager.rainbow(100, (int) rainbowSpeed);
        }
        
        
        
    	boolean players = ESP2D.Players.isEnabled();
    	boolean entitys =  ESP2D.Entitys.isEnabled();
    	
        if (ESP2D.isEnabled && mode.equals("Outline")) {
            ...
            //    mainModel.render(p_77036_1_, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
              ...
              // mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
                ...
                OutlineUtils.setColor(java.util.Objects.requireNonNull(cfo));
                mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
                OutlineUtils.renderFive();
               // OutlineUtils.setColor(Color.WHITE);
            } 
            else if(ESP2D.isEnabled && entitys) {
            ...
            if(Chams.isEnabled && entitylivingbaseIn instanceof EntityPlayer) {
                GL11.glEnable(32823);
                GL11.glPolygonOffset(1.0f, -1000000.0f);
            }
       ...
       ESP2D esp = new ESP2D();
        ...
        float r = (float) esp.red.getValue();
        float g = (float) esp.green.getValue();
        float b = (float) esp.blue.getValue();
        boolean flag2 = esp.isEnabled();
     
        

        if(esp.mode.equalsIgnorecase("Glow") && ModuleManager.get("ESP").isEnabled() && entitylivingbaseIn instanceof EntityPlayer && entitylivingbaseIn != Minecraft.getMinecraft().thePlayer && (players)) {
                ...
                final Color hC = new Color(r  / 255.0F, g / 255.0F, b / 255.0F, 255 / 255.0F);
                int hiddenRed = hC.getRed();
                int hiddenGreen = hC.getGreen();
                int hiddenBlue = hC.getBlue();
                final Color visibleColor = new Color(r  / 255.0F, g / 255.0F, b / 255.0F, 255 / 255.0F);
                ...
                final Color vC = new Color(r  / 255.0F, g / 255.0F, b / 255.0F, 255 / 255.0F);
                int visibleRed = vC.getRed();
                int visibleGreen = vC.getGreen();
                int visibleBlue = vC.getBlue();
                
                 	
                	RenderUtil.glColor(new Color(r  / 255.0F, g / 255.0F, b / 255.0F, 255 / 255.0F).getRGB());
    ...
    public void renderName(T entity, double x, double y, double z)
    {
    	...
    	EventManager.onRenderNametag(event);
