...
public abstract class Render<T extends Entity> {
	...
	protected void renderName(T entity, double x, double y, double z) {
		if (this.canRenderName(entity)) {
			if (!trivia.getModuleByName("NoRender").isToggled() && !ClientSettings.HOLOGRAM) {
	...
	protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
		...
		if (entityIn instanceof EntityPlayer) {
			if(trivia.devTagNames.get(entityIn.getName()) != null) {
				str = trivia.devTagNames.get(entityIn.getName()) + str;
			}
		}
		
		if (trivia.getIsTagName(entityIn.getName())) {
		...
		if (d0 <= (double) (maxDistance * maxDistance)) {
			...
			if (trivia.getModuleByName("NameTags").isToggled() && entityIn instanceof EntityPlayer) {
			 ...
			 */
			if (trivia.getIsMouseHead(entityIn.getName()) ) {
				b0 = -10;
			}
			...
			if(entityIn instanceof EntityPlayer  && trivia.getModuleByName("NameTags").isToggled()) {
