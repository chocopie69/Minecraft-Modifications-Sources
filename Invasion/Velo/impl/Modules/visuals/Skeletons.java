...
public class Skeletons extends Module {
    ...
    public Skeletons() {
        super("Skeletons", "Skeletons", Keyboard.KEY_NONE, Category.VISUALS);
        loadSettings(smoothLines);
    }
    ...
    public void onRender3DUpdate(EventRender3D event) {
          ...
          for (int i = 0, playersLength = players.length; i < playersLength; i++) {
              ...
              if (entPos != null && player.getEntityId() != -1488 && player.isEntityAlive() && this.isInViewFrustrum(player) && !player.isDead && player != mc.thePlayer && !player.isPlayerSleeping() && !player.isInvisible()) {

                  glPushMatrix();
                  final float[][] modelRotations = playerRotationMap.get(player);
                  glLineWidth(1.0f);
                  glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
                  final double x = RenderUtil.interpolate(player.posX, player.lastTickPosX, mc.timer.elapsedPartialTicks) - mc.getRenderManager().viewerPosX;
                  final double y = RenderUtil.interpolate(player.posY, player.lastTickPosY, mc.timer.elapsedPartialTicks) - mc.getRenderManager().viewerPosY;
                  final double z = RenderUtil.interpolate(player.posZ, player.lastTickPosZ, mc.timer.elapsedPartialTicks) - mc.getRenderManager().viewerPosZ;
                  glTranslated(x, y, z);
