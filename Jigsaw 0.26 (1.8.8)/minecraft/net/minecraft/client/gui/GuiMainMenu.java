...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	public void initGui() {
		...
		if (!Jigsaw.ghostMode) {
			int add = 5;
			// this.buttonList.add(new GuiButton(69, 5, this.height - 25, 98,
			// 20, "Alt Login"));
			this.buttonList
					.add(new GuiButton(1337, width - 105, 0 + add, 100, 20, Jigsaw.getClientName().split(" ")[0]));
			// this.buttonList.add(new GuiButton(426, width - 170, 0, 170, 20,
			// "How to Use Infinite Reach"));
			// this.buttonList.add(new GuiButton(420, width - 100, 0 + 22, 100,
			// 20, "Changelog"));
			// this.buttonList.add(new GuiButton(421, width - 100, 22 + 22, 100,
			// 20, "Bugs"));
			// this.buttonList.add(new GuiButton(422, width - 100, 44 + 22, 100,
			// 20, "Suggestions"));
			// this.buttonList.add(new GuiButton(423, width - 100, 44 + 22, 100,
			// 20, "Credits"));
			// this.buttonList.add(new GuiButton(424, width - 100, 22 + 22, 100,
			// 20, "YouTube"));
			this.buttonList.add(new GuiButton(429, width - 65, 22 + add, 60, 20, "Particles"));
			this.buttonList.add(new GuiButton(5, width - 65, 44 + add, 60, 20, "Language"));
			this.buttonList.add(new GuiButton(428, width - 65, 66 + add, 60, 20, "Modules"));
			this.buttonList.add(new GuiButton(427, width - 65, 88 + add, 60, 20, "MCLeaks"));
		...
		if (Jigsaw.ghostMode) {
			...
			this.buttonList.add(new GuiButton(4, this.width - 70, this.height - 25, 65, 20,
	...
	private void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_) {
		if (Jigsaw.ghostMode) {
			...
			this.buttonList.add(new GuiButton(1, 110 - 105, this.height - 25, 98, 20,
					I18n.format("menu.singleplayer", new Object[0])));
			this.buttonList.add(new GuiButton(2, 215 - 105, this.height - 25, 98, 20,
					I18n.format("menu.multiplayer", new Object[0])));
	...
	ArrayList<Object> modules = Jigsaw.filterModulesByCategory(Jigsaw.getModules(), Jigsaw.defaultCategories,
			ReturnType.MODULE);
	float wait = 0;
	int rand1 = 0;
	int rand2 = 0;
	...
	Animation anim = new Animation() {
		...
		public void render() {
			...
			GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
			GlStateManager.color(0f, 0f, 0f, (float) time / 10.0f);
	...
	Animation anim2 = new Animation() {
		...
		public void render() {
			...
			// worldrenderer.pos((double) width - 9, (double) height / 2 + 4,
			// 0.0D).endVertex();
			// worldrenderer.pos((double) width - 1, (double) height / 2,
			// 0.0D).endVertex();
			// worldrenderer.pos((double) width - 9, (double) height / 2 - 4,
			// 0.0D).endVertex();
			// tessellator.draw();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!Jigsaw.ghostMode) {
			...
			if (!particles.isEmpty()) {
				...
				boolean c = false;
				for (MenuParticle particle : particles) {
					GlStateManager.color(c ? 0f : 1f, c ? 0f : 1f, c ? 0f : 1f, particle.alpha);
					double x = particle.x;
					double y = particle.y;
			...
			this.drawTexturedModalRect(j + 12, this.height / 2 - 145, 0, 0, 512 / 2, 512 / 2);

			GL11.glColor4f(1f, 0.95f, 0.95f, 1f);

			this.drawTexturedModalRect(j + 10, this.height / 2 - 147, 0, 0, 512 / 2, 512 / 2);

			GlStateManager.popMatrix();

			this.drawGradientRect(0, 0, this.width, this.height, 0x1fcc7070, 0x1f7070cc);
			
			Fonts.font18.drawStringWithShadow(" - Exploiting Minecraft, with style.", j + 133, this.height / 2 + 18, 0xffffffff);
		...
		// MathHelper.sin((float) (Minecraft.getSystemTime() % 1000L) / 1000.0F
		...
		if (!Jigsaw.ghostMode && !Jigsaw.firstStart && Jigsaw.motd != null) {
			Fonts.font55.drawStringWithShadow("§7Info: §r" + Jigsaw.motd,
					(this.width / 2) - Fonts.font55.getStringWidth("§7Info: §r" + Jigsaw.motd) / 2, this.height - 15 - offset,
					-1);
		}
		if (Jigsaw.firstStart) {
			{
				...
				this.drawString(this.fontRendererObj, str,
						(this.width / 2) - this.fontRendererObj.getStringWidth(str) / 2, this.height - 25 - offset, -1);
			}
			{
				String str = "If you didn't, your computer might be infected with malware!";
				this.drawString(this.fontRendererObj, str,
						(this.width / 2) - this.fontRendererObj.getStringWidth(str) / 2, this.height - 15 - offset, -1);
			}
		}
		...
		if (!Jigsaw.ghostMode) {
			...
			// Jigsaw.getClientName() + " currently has " + modules.size() + "
			// usable hacks!", 5, 5, -1);
			Fonts.font18.drawStringWithShadow("§c" + mc.session.getUsername(), 5, 59, 0xffffffff);
			...
			if (accountInfo || accountInfoOverride) {
				if (mc.session.getSessionType() == Type.LEGACY || mc.session.getUsername().isEmpty()) {
					Fonts.font18.drawStringWithShadow("§7" + "Not Migrated", 5, 59 + 11, 0xffffffff);
				} else {
					Fonts.font18.drawStringWithShadow("§7" + "Migrated (Mojang Account)", 5, 59 + 11, 0xffffffff);
				}
			}
			// GlStateManager.pushMatrix();
			// GlStateManager.scale(1.5, 1.5, 0);
			Fonts.font25.drawStringWithShadow("Alt manager", 62, 4, 0xffffefef);
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (mouseX > 4 && mouseX < 56 && mouseY > 16 && mouseY < 68) {
