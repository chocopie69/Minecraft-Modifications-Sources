...
public class GuiIngame extends Gui {
   ...
   public void renderGameOverlay(float partialTicks) {
      ...
      if (this.recordPlayingUpFor > 0) {
         ...
         if (l1 > 8) {
            ...
            this.getFontRenderer().drawString(this.recordPlaying, (float)(-this.getFontRenderer().getStringWidth(this.recordPlaying) / 2), -4.0F, j2 + (l1 << 24 & -16777216));
            GlStateManager.disableBlend();
      ...
      Massacre.INSTANCE.getEventManager().post(eventRender2D);
      GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
   ...
   public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
      ...
      if (this.mc.thePlayer.experienceLevel > 0) {
         ...
         boolean flag = false;
         this.getFontRenderer().drawString(s, (float)(i1 + 1), (float)l, 0);
         this.getFontRenderer().drawString(s, (float)(i1 - 1), (float)l, 0);
         this.getFontRenderer().drawString(s, (float)i1, (float)(l + 1), 0);
         this.getFontRenderer().drawString(s, (float)i1, (float)(l - 1), 0);
         this.getFontRenderer().drawString(s, (float)i1, (float)l, j1);
         this.mc.mcProfiler.endSection();
      }
   ...
   private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
      ...
      String s;
      for(Iterator var8 = arraylist1.iterator(); var8.hasNext(); i = Math.max(i, this.getFontRenderer().getStringWidth(s))) {
         Object score = var8.next();
         ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(((Score)score).getPlayerName());
         s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, ((Score)score).getPlayerName()) + ": " + EnumChatFormatting.RED + ((Score)score).getScorePoints();
      }
      ...
      int k = 0;
      Iterator var13 = arraylist1.iterator();

      while(var13.hasNext()) {
         Object score1 = var13.next();
         ++k;
         ...
         this.getFontRenderer().drawString(s1, (float)j, (float)l, 553648127);
         this.getFontRenderer().drawString(s2, (float)(i1 - this.getFontRenderer().getStringWidth(s2)), (float)l, 553648127);
         if (k == arraylist1.size()) {
            ...
            this.getFontRenderer().drawString(s3, (float)(j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2), (float)(l - this.getFontRenderer().FONT_HEIGHT), 553648127);
         }
      }
