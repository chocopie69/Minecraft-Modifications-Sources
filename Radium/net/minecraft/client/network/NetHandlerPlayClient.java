...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
            ...
            if (entity instanceof EntityLivingBase && !(entity instanceof EntityPlayerSP)) {
                ...
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                    ...
                    if (dif >= 0.0f && newHealth != 10.0f) {
                        RadiumClient.getInstance().getEventBus().post(new DamageEntityEvent(elb, dif));
                    }
                }
            }
    ...
    public void handleDisconnect(final S40PacketDisconnect packetIn) {
        ...
        final DisconnectEvent disconnectEvent = new DisconnectEvent(packetIn.getReason().getUnformattedText());
    ...
    public void handleRespawn(final S07PacketRespawn packetIn) {
        ...
        if (packetIn.getDimensionID() != this.gameController.thePlayer.dimension) {
            ...
            (this.clientWorldController = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, this.gameController.theWorld.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()), packetIn.getDimensionID(), packetIn.getDifficulty())).setWorldScoreboard(scoreboard);
            this.gameController.loadWorld(this.clientWorldController);
    ...
    public void handleTitle(final S45PacketTitle packetIn) {
        ...
        final DisplayTitleEvent event = new DisplayTitleEvent(StringUtils.stripControlCodes(s3));
    ...
    public void handlePlayerListItem(final S38PacketPlayerListItem packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        for (final S38PacketPlayerListItem.AddPlayerData addPlayerData : packetIn.func_179767_a()) {
    ...
    public void handleResourcePack(final S48PacketResourcePackSend packetIn) {
        ...
        if (!validateResourcePackUrl(this, s, s2)) {
            return;
        }
    ...
    public void sendPacket(final Packet<?> p) {
        final PacketSendEvent sendEvent = new PacketSendEvent(p);
    ...
    private void spawnParticleEntityEvent(final Entity entity, final int type) {
        ...
        if (entity instanceof EntityPlayerSP) {
            ...
            final SpawnParticleEntityEvent event = new SpawnParticleEntityEvent(type, 1);
            RadiumClient.getInstance().getEventBus().post(event);
    ...
    private static boolean validateResourcePackUrl(final NetHandlerPlayClient client, String url, final String hash) {
        try {
            ...
            Wrapper.addChatMessage(String.valueOf(EnumChatFormatting.RED.toString()) + EnumChatFormatting.BOLD + "Cancelled malicious resource download!");
            return false;
        }
        catch (URISyntaxException | UnsupportedEncodingException ex2) {
            final Exception ex;
            final Exception var7 = ex;
            return false;
        }
    }
