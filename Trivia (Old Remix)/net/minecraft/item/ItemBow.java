...
public class ItemBow extends Item {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityPlayer playerIn, int timeLeft) {
		...
		if (flag || playerIn.inventory.hasItem(Items.arrow)) {
			...
			int j = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId+ 99, stack);

			if (j > 0) {
				entityarrow.setDamage(entityarrow.getDamage() + 99 + (double) j * 0.5D + 0.5D);
				trivia.chatMessage(entityarrow.getDamage());
			}
