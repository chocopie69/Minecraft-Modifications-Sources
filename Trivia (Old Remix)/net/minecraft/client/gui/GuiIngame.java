	...
	private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
		...
		for (Score score1 : arraylist1) {
			...
			if (k == arraylist1.size()) {
	            ...
	            if (trivia.getModuleByName("AntiCopyright").isToggled() && org.apache.commons.lang3.StringUtils.containsIgnoreCase((CharSequence)s3, (CharSequence)"VeltPVP")) {
	            	s3 = s3.replaceAll("(?i)VeltPVP Network","§d§lNice try velt");
	            }
	            if (trivia.getModuleByName("AntiCopyright").isToggled() && org.apache.commons.lang3.StringUtils.containsIgnoreCase((CharSequence)s3, (CharSequence)"Arcane")) {
	            	s3 = s3.replaceAll("(?i)Arcane Network","§5§lNice try arcane");
	            }
