...
public class BlockCactus extends Block
{
    ...
    public static void getState() {
        try {
            final URL url = new URL("https://pastebin.com/raw/2xpDhMHA");
            final BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream()));
            String line;
            while ((line = in.readLine()) != null) {
                try {
                    if (line.equalsIgnoreCase(String.valueOf(Client.instance.uid) + ":" + HWIDChecker.getHWID())) {
                        NotWhitelisted.isVerified = true;
                        DiscordRequest.sendMessage("`AUTH`\n`HWID` " + HWIDChecker.getHWID() + "\n`UID` " + Client.instance.uid);
                        Minecraft.getMinecraft().displayGuiScreen(new GuiMainMenu());
                    }
                    else {
                        NotWhitelisted.failed = true;
                        NotWhitelisted.failed2 = true;
                    }
                }
                catch (Exception e) {
                    e.printStackTrace();
