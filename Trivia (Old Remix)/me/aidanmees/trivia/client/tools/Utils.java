...
public class Utils {
	...
	static double x;
	static MarisaTimer timer;
	static double y;
	static int state = 0;
	
	static double z;
	static int cxd = -10000000;
	...
	public static boolean infiniteReach(double range, double maxXZTP, double maxYTP, ArrayList<Vec3> positionsBack,
		...
		// preInfiniteReach(range, maxXZTP, maxYTP, positionsBack, positions,
		// new Vec3(en.posX, en.posY, en.posZ), tpStraight, up, attack,
		// tpUpOneBlock, sneaking);
		double step = maxXZTP / range;
		int steps = 0;
		for (int i = 0; i < range; i++) {
			steps++;
			// trivia.chatMessage(maxXZTP * steps);
	...
	public static boolean infiniteReach(double range, double maxXZTP, double maxYTP, ArrayList<Vec3> positionsBack,
		...
		boolean hit = false;
		boolean tpStraight = false;
		boolean sneaking = mc.thePlayer.isSneaking() || trivia.getModuleByName("AutoSneak").isToggled();
		MovingObjectPosition rayTrace = null;
		MovingObjectPosition rayTrace1 = null;
		MovingObjectPosition rayTraceCarpet = null;
		if ((rayTraceWide(new Vec3(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ), new Vec3(posX, posY, posZ),
				false, false, true))
				|| (rayTrace1 = rayTracePos(
						new Vec3(mc.thePlayer.posX, mc.thePlayer.posY + mc.thePlayer.getEyeHeight(), mc.thePlayer.posZ),
						new Vec3(posX, posY + mc.thePlayer.getEyeHeight(), posZ), false, false, true)) != null) {
			if ((rayTrace = rayTracePos(new Vec3(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ),
				...
				MovingObjectPosition ent = rayTracePos(
						new Vec3(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ), new Vec3(posX, posY, posZ),
						false, false, false);
				if (ent != null && ent.entityHit == null) {
					y = mc.thePlayer.posY;
					yPreEn = mc.thePlayer.posY;
			...
			if (i != steps - 1) {
				...
				if (Math.abs(yDist) < maxYTP && mc.thePlayer.getDistance(posX, posY, posZ) >= 4) {
					...
					mc.getNetHandler().getNetworkManager().sendPacket(
							new C07PacketPlayerDigging(Action.START_DESTROY_BLOCK, targetBlockPos, EnumFacing.UP));
					mc.getNetHandler().getNetworkManager().sendPacket(
							new C07PacketPlayerDigging(Action.STOP_DESTROY_BLOCK, targetBlockPos, EnumFacing.UP));
				} else {
					attack = false;
				}
			}
		...
		if ((rayTraceWide(new Vec3(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ),
			...
			if ((rayTrace = rayTracePos(new Vec3(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ),
					...
					|| (rayTrace1 = rayTracePos(
							new Vec3(mc.thePlayer.posX, mc.thePlayer.posY + mc.thePlayer.getEyeHeight(),
									mc.thePlayer.posZ),
							new Vec3(dest.xCoord, mc.thePlayer.posY + mc.thePlayer.getEyeHeight(), dest.zCoord), false,
							false, true)) != null) {
				MovingObjectPosition trace = null;
				if (rayTrace == null) {
					trace = rayTrace1;
	...
	private static void attackInf(EntityLivingBase en) {
		...
		mc.getNetHandler().getNetworkManager()
				.sendPacketFinal(new C02PacketUseEntity(en, C02PacketUseEntity.Action.ATTACK));
	...
	public static ArrayList<EntityLivingBase> getClosestEntities(float range) {
		...
		for (Object o : Minecraft.getMinecraft().theWorld.loadedEntityList) {
			if (isNotItem(o) && !(o instanceof EntityPlayerSP)) {
				EntityLivingBase en = (EntityLivingBase) o;
				if (en instanceof EntityPlayer  && (ClientSettings.TEAMMM && !Helper.isOnSameTeam(en, mc.thePlayer, true))) {
	...
	// public static boolean checkEntity(boolean friends, boolean invisible,
	// boolean players) {
	// if (en.isEntityEqual(Minecraft.getMinecraft().thePlayer)) {
	// return false;
	// }
	// if (en instanceof EntityPlayer &&
	// trivia.getModuleByName("Freecam").isToggled()
	...
	// }
	// if (en instanceof EntityPlayer &&
	// trivia.getFriendsMananger().isFriend((EntityPlayer) en)) {
	// if (!trivia.getModuleByName("Friends").isToggled()) {
	// return false;
	// }
	// }
	// if (en.isInvisible()) {
	// if (!trivia.getModuleByName("Invisible").isToggled()) {
	// return false;
	// }
	// }
	// if (en instanceof EntityPlayer) {
	// if (!trivia.getModuleByName("Players").isToggled() || en.height < 0.21f)
	// {
	// return false;
	// }
	// }
	// if (Team.isOnTeam(en)) {
	// if (!trivia.getModuleByName("Team").isToggled()) {
	// return false;
	// }
	// }
	// if (!(en instanceof EntityPlayer)) {
	// if (!trivia.getModuleByName("NonPlayers").isToggled()) {
	// return false;
	// }
	// }
	// if ((en instanceof EntityPlayer)) {
	// if (trivia.getBypassManager().getEnabledBypass() != null &&
	// trivia.getBypassManager().getEnabledBypass().getName().equals("AntiGwen"))
	// {
	...
	// }
	// if(trivia.getBypassManager().getEnabledBypass() != null &&
	// trivia.getBypassManager().getEnabledBypass().getName().equals("AntiWatchdog"))
	// {
	...
	// }
	// // if(en.hurtTime > 12 &&
	// // !trivia.getModuleByName("HurtResistant").isToggled()) {
	// // return false;
	// // }
	// return true;
	// }
	...
	public static boolean validEntity(EntityLivingBase en) {
		...
		if (en instanceof EntityPlayer && trivia.getModuleByName("Freecam").isToggled()
				&& en.getName().equals(Minecraft.getMinecraft().thePlayer.getName())) {
			return false;
		}
		if (en instanceof EntityPlayer && trivia.getModuleByName("Blink").isToggled()
				&& en.getName().equals(Minecraft.getMinecraft().thePlayer.getName())) {
			return false;
		}
		...
		if (en instanceof EntityPlayer && KillauraBETA.IsntMineplexBot(en) && en.isInvisible() == false && trivia.getFriendsMananger().isFriend((EntityPlayer) en) && !en.isDead && !en.isEntityEqual(Minecraft.getMinecraft().thePlayer)) {
			trivia.chatMessage(en.isInvisible());
				return true;
	...
	public static float col() {
		...
		Color color = new Color(Color.HSBtoRGB((float) (Minecraft.getMinecraft().thePlayer.ticksExisted / 120.0D
				+ Math.sin(cxd / 50.0D * 1.5707963267948966D)) % 1.0F, 0.627451F, 1.0F));
		col = new Color(color.getRed(), color.getGreen(), color.getBlue(), 220).getRGB();
		cxd += 3;
...
public static int ColorBlue(){
	...
	public static int RainbowTestr() {

		Color color = new Color(Color.HSBtoRGB((float) (Minecraft.getMinecraft().thePlayer.ticksExisted / 120.0D
				+ Math.sin(cxd / 50.0D * 1.5707963267948966D)) % 1.0F, 0.627451F, 1.0F));
		int meme = color.getRed();

		return meme;
	}
	public static int RainbowTestg() {

		Color color = new Color(Color.HSBtoRGB((float) (Minecraft.getMinecraft().thePlayer.ticksExisted / 120.0D
				+ Math.sin(cxd / 50.0D * 1.5707963267948966D)) % 1.0F, 0.627451F, 1.0F));
		int meme = color.getGreen();

		return meme;
	}
	public static int RainbowTestb() {

		Color color = new Color(Color.HSBtoRGB((float) (Minecraft.getMinecraft().thePlayer.ticksExisted / 120.0D
				+ Math.sin(cxd / 50.0D * 1.5707963267948966D)) % 1.0F, 0.627451F, 1.0F));
		int meme = color.getBlue();
