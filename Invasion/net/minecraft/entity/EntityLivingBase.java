...
import Velo.impl.Modules.visuals.BloodParticles;
import Velo.impl.Modules.visuals.NoHurtCam;
...
public abstract class EntityLivingBase extends Entity {
	...
	private int getArmSwingAnimationEnd() {
		...
		return this.isPotionActive(Potion.digSpeed)
			...
			return (int) (6 + Animations.hitslowdown.getValue());
		}
	}
	...
	public void handleStatusUpdate(byte id) {
		if (id == 2) {
			...
			if(!NoHurtCam.isEnabled) {
				...
				if(!this.isEntityEqual(Minecraft.getMinecraft().thePlayer)) {
					this.hurtTime = this.maxHurtTime = 10;
				}
			}
			this.attackedAtYaw = 0.0F;
			...
			if(BloodParticles.isEnabled) {
				if(!this.isEntityEqual(Minecraft.getMinecraft().thePlayer)) {
					for(int i = 0; i < 15; i++) {
						this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, this.getEntityBoundingBox().minY + 1D, this.posZ + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, -this.motionX * 4.0D, 1.5D, -this.motionZ * 4.0D, 152);
					}
				}
			}
			
			if(SoundModifier.isEnabled) {
				if(SoundModifier.bloodparticlessound.isEnabled()) {
					if(!this.isEntityEqual(Minecraft.getMinecraft().thePlayer) && this.getDistanceToEntity(Minecraft.getMinecraft().thePlayer) < 15) {
						Minecraft.getMinecraft().thePlayer.playSound("dig.stone", this.getSoundVolume(),
								(100));
					}
				}
			}
			...
			if (s != null) {
				if(!SoundModifier.isEnabled) {
					...
					if(SoundModifier.hurtsound.isEnabled()) {
						this.playSound(this.getHurtSound(), this.getSoundVolume(),
								(this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
					}
				}
			...
			if (s1 != null) {
				if(!SoundModifier.isEnabled) {
					...
					if(SoundModifier.deathsound.isEnabled()) {
						this.playSound(this.getDeathSound(), this.getSoundVolume(),
								(this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
					}
				}
	 ...
	 */
	public Vec3 getLookVec(float partialTicks) {
