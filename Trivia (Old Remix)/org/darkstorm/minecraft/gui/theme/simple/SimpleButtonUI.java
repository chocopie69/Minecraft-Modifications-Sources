...
public class SimpleButtonUI extends AbstractComponentUI<Button> {
	...
	protected void renderComponent(Button button) {
		...
		boolean searching = trivia.getSearchBar() != null && !trivia.getSearch().isEmpty();
		if (searching) {
			searched = button.getText().toUpperCase().indexOf(trivia.getSearch().toUpperCase()) != -1;
		}
...
//			if(!trivia.isModAllowed(bscButton.getModule())) {
...
//		if(button instanceof BasicButton) {
//			BasicButton bscButton = (BasicButton)button;
//			allowed = trivia.isModAllowed(bscButton.getModule());
//		}
		if(allowed) {
