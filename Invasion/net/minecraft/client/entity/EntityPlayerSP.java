...
import Velo.impl.Modules.player.NoSlowDown;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public static TargetStrafe ta = new TargetStrafe();
    ...
    public void setSpeed2(double speed) {
        ...
        boolean targetStrafe = ta.canStrafe();
        if (targetStrafe) {
        ...
        double yaw = targetStrafe ? ka.getKaRotations(ta.entity)[0] : ka.yaw;
	...
	public void hClip(double clipRange) {

		mc.thePlayer.setPosition(mc.thePlayer.posX + mc.thePlayer.getLookVec().xCoord * clipRange, mc.thePlayer.posY,
				mc.thePlayer.posZ + mc.thePlayer.getLookVec().zCoord * clipRange);

	}

	public void PackethClip(double clipRange, boolean onGround) {

		mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(
				mc.thePlayer.serverPosX + mc.thePlayer.getLookVec().xCoord * clipRange, mc.thePlayer.posY,
				mc.thePlayer.serverPosZ + mc.thePlayer.getLookVec().zCoord * clipRange, onGround));
	...
	public void vClip(double clipRange) {

		mc.thePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY + clipRange, mc.thePlayer.posZ);

	}

	public void PacketvClip(double clipRange, boolean onGround) {

		mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX,
				mc.thePlayer.serverPosY + clipRange, mc.thePlayer.posZ, onGround));
	...
	public boolean isMovingWhenOnGround() {
	...
	public void onUpdate() {
		if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			...
			if (this.isRiding()) {
				...
				EventPostMotion event = new EventPostMotion(this.posX, this.getEntityBoundingBox().minY, this.posZ,
						this.rotationYaw, this.rotationPitch, this.onGround);
				EventManager.onPostMotionUpdate(event);
			}
		}
	}
	...
	public void onUpdateWalkingPlayer() {
		...
		EventPreMotion event = new EventPreMotion(this.lastReportedYaw, this.lastReportedPitch,this.posX, this.getEntityBoundingBox().minY, this.posZ,
                this.rotationYaw, this.rotationPitch, this.onGround);
		EventManager.onPreMotionUpdate(event);
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getX(), event.getY(),
							event.getZ(), event.getYaw(), event.getPitch(), event.isOnGround()));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.getX(),
							event.getY(), event.getZ(), event.isOnGround()));
				} else if (flag3) {
    ...
    public void moveEntity(double x, double y, double z) {
    	EventMovement event = new EventMovement(x, y, z);
    	EventManager.onMovementUpdate(event);
  //  	ChatUtil.addChatMessage("asdfasdfsd");
	...
	public void sendChatMessage(String message) {
		...
		EventManager.onEventChatMessage(event);
		if (event.isCancelled())
			return;
	...
	public void onLivingUpdate() {
		...
		if (this.sprintingTicksLeft > 0 && !NoSlowDown.isEnabled) {
		...
		if (this.sprintToggleTimer > 0 && !NoSlowDown.isEnabled) {
		...
		if (this.isUsingItem() && !this.isRiding() && !NoSlowDown.isEnabled) {
