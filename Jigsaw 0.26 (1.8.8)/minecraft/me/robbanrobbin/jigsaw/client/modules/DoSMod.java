...
public class DoSMod extends Module {
	...
	@Override
	public void onGui() {
		Jigsaw.getUIRenderer().addToQueue("§cDoS Attack: §r" + packetCount.get() + " packets sent!", ScreenPos.LEFTUP);
		if(type == 1) {
			Jigsaw.getUIRenderer().addToQueue(
	...
	public static void startFlooding(final InetAddress addrrrrr, final byte[] bytes, final Random rand,
		...
		final int port = ServerAddress.func_78860_a(mc.getCurrentServerData().serverIP).getPort();
