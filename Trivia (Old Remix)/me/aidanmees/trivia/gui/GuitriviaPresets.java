...
import me.aidanmees.trivia.client.presets.ModulePreset;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.GuiButton;
import net.minecraft.client.gui.GuiScreen;

public class GuitriviaPresets extends GuiScreen {
	private GuiScreen before;
	private int i = 0;

	// private GuiTextField presetNameField;
	public GuitriviaPresets(GuiScreen before) {
		this.before = before;
	}

	public void initGui() {
		for (ModulePreset mp : trivia.getPresetMananger().getPresets()) {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id > i) {
			if (button.id == i + 1) {
				trivia.getPresetMananger().getPresets().add(new ModulePreset(trivia.getToggledModulesObject(),
						String.valueOf(trivia.getPresetMananger().getPresets().size() + 1)));
				Minecraft.getMinecraft().displayGuiScreen(new GuitriviaPresets(before));
			}
			if (button.id == i + 2) {
				for (ModulePreset mp : trivia.getPresetMananger().getPresets()) {
			...
			trivia.getPresetMananger().getPresetByName(((GuiButton) this.buttonList.get(button.id)).displayString)
					.toggle();
		}
