...
public class GuiTextField extends Gui
{
    ...
    public void mouseClicked(int mouseX, int mouseY){
        if(mouseX > xPosition && mouseX < (length > 50 ? xPosition + length : xPosition + fontRendererInstance.getStringWidth(name)) && mouseY > yPosition && mouseY < yPosition + fontRendererInstance.FONT_HEIGHT){
            this.isFocused = true;
        }else {
    ...
    public void drawTextBox()
    {
        if (this.getVisible())
        {
            ...
            if (s.length() > 0)
            {
               ...
               // Artemis.instance.cfrs.drawStringWithShadow(s1, (float)l, (float)i1, i);
            }
    ...
    public void draw(){
        ...
        float diff = fontRendererInstance.getStringWidth(text) - length;
        length += diff / 4;
        if(fontRendererInstance.getStringWidth(text) < 50){
            Gui.drawRect(xPosition, yPosition + fontRendererInstance.FONT_HEIGHT + 3, xPosition + 50 + fontRendererInstance.getStringWidth(name) - 40, yPosition + fontRendererInstance.FONT_HEIGHT + 4, -1);
        }else {
            Gui.drawRect(xPosition, yPosition + fontRendererInstance.FONT_HEIGHT + 3, xPosition + length + 7, yPosition + fontRendererInstance.FONT_HEIGHT + 4, -1);
        }
        if(text.equalsIgnoreCase("") && !isFocused){
        	fontRendererInstance.drawString(name, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
        }
        if(!timer.elapsed(1000)) {
            if(timer.elapsed(500) && isFocused) {
            	fontRendererInstance.drawString(text + "|", xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
               // fontRendererInstance.getStringHeight.drawString(text, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
            }else {
            	fontRendererInstance.drawString(text, xPosition, yPosition + 1, new Color(160, 160, 160, 255).getRGB());
            }
        }

        if(timer.elapsed(950)){
            timer.reset1();
        }
        GlStateManager.color(1.0f, 1.0f,1.0f,1.0f);
