...
public class Gui
{
    ...
    public static void drawRect(double e, double theOffset, double g, double d, final int color) {
        if (e < g) {
            final double i = e;
            e = g;
            g = i;
        ...
        worldrenderer.pos(e, d, 0.0).endVertex();
        worldrenderer.pos(g, d, 0.0).endVertex();
        worldrenderer.pos(g, theOffset, 0.0).endVertex();
        worldrenderer.pos(e, theOffset, 0.0).endVertex();
        tessellator.draw();
    ...
    public void drawString(final FontRenderer fontRendererIn, final String text, final float f, final float g, final int color) {
        fontRendererIn.drawStringWithShadow(text, f, g, color);
    }
    ...
    public static void drawModalRectWithCustomSizedTexture(final float g, final float h, final float u, final float v, final int width, final int height, final float textureWidth, final float textureHeight) {
        final float f = 1.0f / textureWidth;
        final float f2 = 1.0f / textureHeight;
        final Tessellator tessellator = Tessellator.getInstance();
        final WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos(g, h + height, 0.0).tex(u * f, (v + height) * f2).endVertex();
        worldrenderer.pos(g + width, h + height, 0.0).tex((u + width) * f, (v + height) * f2).endVertex();
        worldrenderer.pos(g + width, h, 0.0).tex((u + width) * f, v * f2).endVertex();
