...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	public void initGui() {
		...
		if (!Jigsaw.ghostMode) {
			...
			this.buttonList
					.add(new GuiButton(1337, width - 100, 132 + 11, 100, 20, Jigsaw.getClientName().split(" ")[0]));
			...
			this.buttonList.add(new GuiButton(427, width - 100, 66 + 22, 100, 20, "MCLeaks"));
			this.buttonList.add(new GuiButton(429, width - 100, 198 - 22, 100, 20, "Particles"));
			this.buttonList.add(new GuiButton(428, width - 100, 132 + 22 + 44,
			100, 20, "Modules"));
		...
		if (Jigsaw.ghostMode) {
			...
			this.buttonList.add(new GuiButton(5, width - 100, 154 - 44, 100, 20, "Language"));
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 428) {
			mc.displayGuiScreen(new GuiManagerDisplayScreen(Jigsaw.getGUIMananger(), true));
		}
		if (button.id == 429) {
...
//			tessellator.draw();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!Jigsaw.ghostMode) {
			...
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			GlStateManager.scale(width * 0.004, height * 0.004, 0);
			this.drawTexturedModalRect(0, 0, 0, 0, 512 / 2, 512 / 2);
			GlStateManager.popMatrix();
			anim.render();
			...
			if (!particles.isEmpty()) {
				...
				boolean c = true;
				for (MenuParticle particle : particles) {
					//GlStateManager.color((float)particle.motionY, (float)particle.motionX, (float)(particle.motionX / particle.motionY), particle.alpha);
					GlStateManager.color(c ? 0f : 1f, c ? 0f : 1f, 
							c ? 0f : 1f, particle.alpha);
					worldrenderer.begin(7, DefaultVertexFormats.POSITION);
					worldrenderer.pos((double) particle.x, (double) particle.y + 1, 0.0D).endVertex();
					worldrenderer.pos((double) particle.x + 0.5, (double) particle.y + 1, 0.0D).endVertex();
					worldrenderer.pos((double) particle.x + 0.5, (double) particle.y, 0.0D).endVertex();
					worldrenderer.pos((double) particle.x, (double) particle.y, 0.0D).endVertex();
					tessellator.draw();
				}
				GlStateManager.enableTexture2D();
				GlStateManager.disableBlend();
			...
			this.drawTexturedModalRect(j + 12, k - 108, 0, 0, 512 / 2, 512 / 2);
			
			GL11.glColor4f(1f, 1f, 1f, 1f);
			
			this.drawTexturedModalRect(j + 10, k - 110, 0, 0, 512 / 2, 512 / 2);
		...
		if (!Jigsaw.ghostMode && Jigsaw.motd != null) {
