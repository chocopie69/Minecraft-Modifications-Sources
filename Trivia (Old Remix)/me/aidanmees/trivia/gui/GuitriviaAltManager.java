...
public class GuitriviaAltManager extends GuiScreen {
	...
	public GuitriviaAltManager(GuiScreen before) {
		this.before = before;
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id == 2) {
			mc.displayGuiScreen(new GuiAltManager(before));
		}
		if (button.id == 3) {
			if (displayingLogin) {
				stopDisplayingLogin();
		...
		if (button.id == 10) {
			...
			trivia.getAltManager().addAlt(alt);
			stopDisplayingLogin();
		}
		if (button.id == 11) {
			stopDisplayingFailed();
		...
		if(button.id == 1) {
			trivia.getAltManager().getAlts().remove(trivia.getAltManager().getAlts().get(selected));
		}
		if(button.id == 0) {
			Alt alt = trivia.getAltManager().getAlts().get(selected);
			if(alt.cracked) {
				Login.changeName(alt.name);
				trivia.loggedInName = trivia.getAltManager().getAlts().get(selected).name;
			}
			else {
				try {
			...
			if(result == JFileChooser.APPROVE_OPTION) {
				...
				if(file.getName().endsWith(".txt")) {
					...
					try {
						...
						for (Alt alt : trivia.getAltManager().getAlts()) {
							if(alt.cracked) {
								pw.println(alt.name);
							}
							else {
	...
	public void updateScreen() {
		...
		anim.update();
		if(selected >= trivia.getAltManager().getAlts().size()) {
			selected = trivia.getAltManager().getAlts().size() - 1;
		}
		if(!displayingLogin) {
			emailField.setText("");
			passField.setText("");
		}
		this.buttonList.get(0).enabled = !trivia.getAltManager().getAlts().isEmpty();
		this.buttonList.get(1).enabled = !trivia.getAltManager().getAlts().isEmpty();
		super.updateScreen();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRendererObj, trivia.headerNoBrackets, width / 2 / 2, (10) / 4, 0xffffffff);
		...
		for (Alt alt : trivia.getAltManager().getAlts()) {
			...
			if(trivia.loggedInName != null && trivia.loggedInName.equals(alt.name)) {
	...
	public void handleMouseInput() throws IOException {
		...
		if (i != 0) {
			...
			if(this.scroll < (-trivia.getAltManager().getAlts().size() * 21) + 21 * 5) {
				this.scroll = (-trivia.getAltManager().getAlts().size() * 21) + 21 * 5;
			}
		}
	}
