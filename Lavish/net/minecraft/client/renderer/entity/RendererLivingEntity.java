...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer(final Object layerEnderDragonDeath) {
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                float f5;
                if (Client.instance.moduleManager.getModuleByName("Scaffold").isEnabled() && entity.getEntityId() == Minecraft.getMinecraft().thePlayer.getEntityId()) {
                    f5 = RenderUtils.getCustomPitch();
                }
                else if (Client.instance.moduleManager.getModuleByName("Killaura").isEnabled() && entity.getEntityId() == Minecraft.getMinecraft().thePlayer.getEntityId()) {
                    f5 = RenderUtils.getCustomPitch();
                }
                else {
