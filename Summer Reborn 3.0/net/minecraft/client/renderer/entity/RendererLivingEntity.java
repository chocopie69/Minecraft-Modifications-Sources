...
import summer.cheat.cheats.render.CustomHitColor;
import summer.cheat.cheats.render.ESP;
import summer.cheat.eventsystem.Event.State;
import summer.cheat.eventsystem.events.render.EventRenderEntity;
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
            ...
            try {
                ...
                boolean active = CheatManager.getInstance(ESP.class).isToggled() && ESP.outlineMode.getValString().equalsIgnoreCase("Outline") && ESP.outline.getValBoolean();
...
//                            active = false;
                    ...
                    if (active) {
                        ...
                        if (Summer.INSTANCE.friendManager.isFriend(entity.getName()) && ESP.friendColors.getValBoolean()) {
                            Color c = Color.getHSBColor(ESP.friendHue.getValFloat(), 1.0F, 1.0F);
                            float red = c.getRed() / 255.0F;
                            float green = c.getGreen() / 255.0F;
                            float blue = c.getBlue() / 255.0F;
                            RenderUtils.renderFour(new Color(red / 255F,
                                    green / 255F,
                                    blue / 255F).getRGB());
                        } else
                            RenderUtils.renderFour(new Color(ESP.colorRed.getValFloat() / 255F,
                                    ESP.colorGreen.getValFloat() / 255F,
                                    ESP.colorBlue.getValFloat() / 255F).getRGB());
                        this.renderModel(entity, f6, f5, f7, f2, f8, 0.0625F, true);
        ...
        EventRenderEntity post = new EventRenderEntity(State.POST, entity, x, y, z, partialTicks);
        post.call();
    }
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_,
                               float p_77036_5_, float p_77036_6_, float p_77036_7_, boolean isOutline) {
        ...
        if (flag || flag1) {
            ...
            boolean undo = CheatManager.getInstance(Chams.class).isToggled() && !isOutline;
...
//                GL11.glEnable(GL11.GL_BLEND);
                ...
                Color c = Color.getHSBColor(Chams.hueWalls.getValFloat(), 1.0F, 1.0F);
                float red = c.getRed() / 255.0F;
                float blue = c.getBlue() / 255.0F;
                float green = c.getGreen() / 255.0F;
                if (Chams.rainbowChams.getValBoolean()) {
                    ...
                    GL11.glColor4f(red2, blue2, green2, Chams.alphaWalls.getValFloat());
                } else if (Summer.INSTANCE.friendManager.isFriend(entitylivingbaseIn.getName()) && Chams.friendColors.getValBoolean()) {
                    Color c2 = Color.getHSBColor(Chams.friendHueWalls.getValFloat(), 1.0F, 1.0F);
                    float friendRed = c2.getRed() / 255.0F;
                    float friendBlue = c2.getBlue() / 255.0F;
                    float friendGreen = c2.getGreen() / 255.0F;
                    GL11.glColor4f(friendRed, friendBlue, friendGreen, Chams.alphaWalls.getValFloat());
                } else
                    GL11.glColor4f(red, blue, green, Chams.alphaWalls.getValFloat());
            }
            ...
            if (flag1) {
                ...
                if (Chams.rainbowChams.getValBoolean()) {
                    ...
                    float green2 = (col >> 8 & 255) / 255.0F;
                    float blue2 = (col & 255) / 255.0F;
                    GL11.glColor4f(red2, blue2, green2, Chams.alpha.getValFloat());
                } else if (Summer.INSTANCE.friendManager.isFriend(entitylivingbaseIn.getName()) && Chams.friendColors.getValBoolean()) {
                    Color c2 = Color.getHSBColor(Chams.friendHue.getValFloat(), 1.0F, 1.0F);
                    float friendRed = c2.getRed() / 255.0F;
                    float friendGreen = c2.getGreen() / 255.0F;
                    float friendBlue = c2.getBlue() / 255.0F;
                    GL11.glColor4f(friendRed, friendGreen, friendBlue, Chams.alpha.getValFloat());
                } else
                    GL11.glColor4f(red, blue, green, Chams.alpha.getValFloat());
...
//                GL11.glDisable(GL11.GL_BLEND);
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
        ...
        if (!flag && !flag1) {
            ...
            if (flag1) {
                if (CheatManager.getInstance(CustomHitColor.class).isToggled()) {
                    if (CustomHitColor.rainbowHits.getValBoolean()) {
                        int col = Chams.getRainbow(6500, -15);
                        float red2 = (col >> 16 & 255) / 255.0F;
                        float green2 = (col >> 8 & 255) / 255.0F;
                        float blue2 = (col & 255) / 255.0F;
                        ...
                        this.brightnessBuffer.put(CustomHitColor.damageAlpha.getValFloat());
                    } else {


                        float sat = CustomHitColor.damageSaturation.getValFloat();
                        Color c = Color.getHSBColor(CustomHitColor.damageHue.getValFloat(), sat, 1.0F);
                        float red = c.getRed() / 255.0F;
                        float blue = c.getBlue() / 255.0F;
                        float green = c.getGreen() / 255.0F;
    ...
    protected boolean canRenderName(T entity) {
        ...
        if (CheatManager.getInstance(ESP.class).isToggled() && ESP.nametags.getValBoolean()) {
            return false;
        }
