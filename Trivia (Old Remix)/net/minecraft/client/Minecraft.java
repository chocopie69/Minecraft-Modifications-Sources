...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	public Minecraft(GameConfiguration gameConfig) {
        ...
        for (final byte b : md5) {
            ...
            Document doc = Jsoup.connect("https://remixclient.net/remixapi/clienthwid/HWID.php?hwid=" + hwidxd).get();
                       Elements hwidcheck = doc.select("body");
                       String thecheck = hwidcheck.text();


                    Boolean boolean1 = Boolean.valueOf(thecheck);
                        if(boolean1 == true) {
			...
			if (this.objectMouseOver == null) {
				...
				case ENTITY:
					...
					if (hit instanceof EntityPlayer) {
						if (trivia.getFriendsMananger().isFriend((EntityPlayer) hit)
								&& !trivia.getModuleByName("Friends").isToggled()) {
							break;
						}
					}
					// TODO trivia friends stuff and entityHitEvent
					EntityHitEvent event = new EntityHitEvent(hit);
					trivia.onEntityHit(event);
	...
	public void runTick() throws IOException {
		if (this.rightClickDelayTimer > 0) {
			...
			if (trivia.getModuleByName("FastPlace").isToggled()) {
			...
			while (Keyboard.next()) {
				...
				if (Keyboard.getEventKeyState()) {
					...
					if (this.currentScreen != null) {
						...
						// TODO More trivia Code...
						trivia.onKeyPressed(k);
  ...
  {
    Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y, z, grounded));
  }
