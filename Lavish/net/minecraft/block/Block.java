...
public class Block
{
    ...
    public void addCollisionBoxesToList(final World worldIn, final BlockPos pos, final IBlockState state, final AxisAlignedBB mask, final List<AxisAlignedBB> list, final Entity collidingEntity) {
        this.axisalignedbb = this.getCollisionBoundingBox(worldIn, pos, state);
        final EventCollide eventCollide = new EventCollide(collidingEntity, pos.getX(), pos.getY(), pos.getZ(), this.axisalignedbb, this);
        Client.instance.onEvent(eventCollide);
        this.axisalignedbb = eventCollide.getBoundingBox();
        if (this.axisalignedbb != null && mask.intersectsWith(this.axisalignedbb)) {
            list.add(this.axisalignedbb);
        }
    }
