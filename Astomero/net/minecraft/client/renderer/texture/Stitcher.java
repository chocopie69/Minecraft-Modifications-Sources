...
public class Stitcher
{
    ...
    public void addSprite(final TextureAtlasSprite p_110934_1_) {
        final Holder stitcherholder = new Holder(p_110934_1_, this.mipmapLevelStitcher);
    ...
    public void doStitch() {
        final Holder[] astitcherholder = this.setStitchHolders.toArray(new Holder[this.setStitchHolders.size()]);
        Arrays.sort(astitcherholder);
        for (final Holder stitcherholder : astitcherholder) {
            if (!this.allocateSlot(stitcherholder)) {
                final String s = String.format("Unable to fit: %s, size: %dx%d, atlas: %dx%d, atlasMax: %dx%d - Maybe try a lower resolution resourcepack?", stitcherholder.getAtlasSprite().getIconName(), stitcherholder.getAtlasSprite().getIconWidth(), stitcherholder.getAtlasSprite().getIconHeight(), this.currentWidth, this.currentHeight, this.maxWidth, this.maxHeight);
    ...
    public List getStichSlots() {
        ...
        for (final Object stitcherslot : this.stitchSlots) {
            ((Slot)stitcherslot).getAllStitchSlots(arraylist);
        }
        final ArrayList arraylist2 = Lists.newArrayList();
        for (final Object stitcherslot2 : arraylist) {
            final Slot stitcherslot3 = (Slot)stitcherslot2;
            final Holder stitcherholder = stitcherslot3.getStitchHolder();
            final TextureAtlasSprite textureatlassprite = stitcherholder.getAtlasSprite();
            textureatlassprite.initSprite(this.currentWidth, this.currentHeight, stitcherslot3.getOriginX(), stitcherslot3.getOriginY(), stitcherholder.isRotated());
    ...
    private boolean expandAndAllocateSlot(final Holder p_94311_1_) {
        ...
        if (flag6) {
            ...
            stitcherslot = new Slot(this.currentWidth, 0, p_94311_1_.getWidth(), this.currentHeight);
            this.currentWidth += p_94311_1_.getWidth();
        }
        else {
            stitcherslot = new Slot(0, this.currentHeight, this.currentWidth, p_94311_1_.getHeight());
            this.currentHeight += p_94311_1_.getHeight();
        }
    ...
    public static class Slot
    {
        ...
        public boolean addSlot(final Holder holderIn) {
            ...
            for (final Object stitcherslot : this.subSlots) {
