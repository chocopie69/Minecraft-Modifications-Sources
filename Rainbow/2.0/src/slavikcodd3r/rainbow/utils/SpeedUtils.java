...
public class SpeedUtils
{
    ...
    public static void setMinemanFastestSpeed(final MoveEvent event, final int tickDelay) {
        ...
        if (SpeedUtils.mc.thePlayer.ticksExisted % ((tickDelay <= 2) ? 3 : tickDelay) == 0) {
        ...
        if (event == null) {
            ...
            setMoveSpeed(event, baseSpeed);
        }
    }
    
    public static void setMoveSpeed(final double moveSpeed) {
        double forward = SpeedUtils.mc.thePlayer.movementInput.moveForward;
        double strafe = SpeedUtils.mc.thePlayer.movementInput.moveStrafe;
        if (SpeedUtils.tsStuff != null) {}
        if (forward > 0.0) {
            forward = 1.0;
        }
        else if (forward < 0.0) {
        ...
        final double yaw = SpeedUtils.mc.thePlayer.rotationYaw;
        ...
        SpeedUtils.mc.thePlayer.motionX = forward * moveSpeed * sin + strafe * moveSpeed * cos;
        SpeedUtils.mc.thePlayer.motionZ = forward * moveSpeed * cos - strafe * moveSpeed * sin;
    }
    ...
    public static void setMoveSpeed(final MoveEvent event, final double moveSpeed) {
        final MovementInput progamermove = SpeedUtils.mc.thePlayer.movementInput;
        double yeetthefuckingbaby = progamermove.moveForward;
        double yeetonniggers = progamermove.moveStrafe;
        double yaw = SpeedUtils.mc.thePlayer.rotationYaw;
    ...
    public static double getBaseMoveSpeed() {
        ...
        if (SpeedUtils.mc.thePlayer != null && Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.moveSpeed)) {
    ...
    public static double getJumpHypixel() {
        ...
        if (SpeedUtils.mc.thePlayer.isPotionActive(Potion.jump)) {
            motionY += (SpeedUtils.mc.thePlayer.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1f;
        }
        return motionY;
    }
