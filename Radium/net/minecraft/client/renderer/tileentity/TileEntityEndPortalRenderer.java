    ...
    public void renderTileEntityAt(final TileEntityEndPortal te, final double x, final double y, final double z, final float partialTicks, final int destroyStage) {
        if (!Config.isShaders() || !ShadersRender.renderEndPortal(te, x, y, z, partialTicks, destroyStage, 0.75f)) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                GL11.glTranslatef((float)ActiveRenderInfo.getPosition().xCoord * f5 / f9, (float)ActiveRenderInfo.getPosition().zCoord * f5 / f9, -f2);
                ...
                worldrenderer.pos(x, y + f4, z).color4f(f12, f13, f14, 1.0f).endVertex();
                worldrenderer.pos(x, y + f4, z + 1.0).color4f(f12, f13, f14, 1.0f).endVertex();
                worldrenderer.pos(x + 1.0, y + f4, z + 1.0).color4f(f12, f13, f14, 1.0f).endVertex();
                worldrenderer.pos(x + 1.0, y + f4, z).color4f(f12, f13, f14, 1.0f).endVertex();
                tessellator.draw();
                GL11.glPopMatrix();
