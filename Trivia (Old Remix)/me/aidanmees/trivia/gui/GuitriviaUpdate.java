...
public class GuitriviaUpdate extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRendererObj, "Your trivia version is outdated!", width / 2, 10 - scroll, 0xffffff);

		drawCenteredString(fontRendererObj, "Your version is: §c" + trivia.getClientVersion(), width / 2, 20 - scroll,
				0xffffff);

		drawCenteredString(fontRendererObj, "Recommended version is: §6" + trivia.serverVersion, width / 2, 30 - scroll,
				0xffffff);

		if (!trivia.changelogFailed) {
			if (trivia.changeLineElmts != null) {
				GlStateManager.pushMatrix();
				GlStateManager.scale(1.8, 1.8, 1);
				drawString(fontRendererObj, "Changelog: ", 2, (int) Math.round(30 - scroll / 1.8), 0xffffff);
				GlStateManager.popMatrix();
				int sub = 80;
				for (Element elmt : trivia.changeLineElmts) {
					if (elmt.ownText().trim().equals(trivia.getClientVersion())) {
						drawString(fontRendererObj, "§c§l" + elmt.ownText() + "§6 - Your version", 5, sub - scroll,
								0xffffff);
					} else if (elmt.ownText().trim().equals(trivia.serverVersion)) {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (id == 0) {
			try {
				this.openWebLink(new URI("https://www.triviaclient.ml/#download"));
				mc.shutdown();
			} catch (URISyntaxException e) {
				mc.displayGuiScreen(new GuitriviaOpenLinkFailed());

			}
		}
		if (id == 1) {
			mc.displayGuiScreen(new GuitriviaUpdate2());
		}
