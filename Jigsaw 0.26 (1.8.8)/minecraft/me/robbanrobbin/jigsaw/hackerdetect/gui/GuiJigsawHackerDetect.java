	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		{
			...
			drawRect(0, 27, 200, height - 22, 0xaf303030); // sidebar with all
															// players
			drawRect(200, 27, width, height - 22, 0xaf101010); // big area to
																// the right
			drawRect(0, height - 22, width, height, 0xcf101010); // bar below
																	// everything
																	// with the
																	// buttons
			...
			String s = HackerDetect.getLoadedPlayers() + " players loaded §8|§r " + HackerDetect.hackers.size()
					+ " players in total";
			Fonts.font18.drawCenteredString("§cJig§8saw§r§7§o Advanced §rHackerDetect", width / 2, 2, 0xffffffff);
			...
			if (selected != null) {
				...
				for (Check check : selected.checks) {
					...
					if (ClientSettings.hackerDetectMoreInfo) {
						Fonts.font18.drawStringWithShadow(
								"§7 - §r" + check.getName() + " §8 -§7 Violated §c" + check.getViolations()
						...
						Fonts.font18.drawStringWithShadow("§7 - §r" + check.getName() + " §8 -§7 Violated §c"
				...
				drawString(Fonts.font18, "AccuracyList" + ": §c" + selected.accuracyValues.size(), padding + 20,
						infoY - 20, 0xffffffff);
				if (selected.isAccuracyListUsable()) {
					drawString(Fonts.font18, "Accuracy(0-360)" + ": §c" + selected.accuracyValue, padding + 20,
							infoY - 10, 0xffffffff);
				}

				drawString(Fonts.font18, "Max APS" + ": §c" + (int) Math.floor((selected.maxAps * 0.8)) + " §7- §c"
						+ (int) Math.ceil((selected.maxAps * 2)), padding + 20, infoY, 0xffffffff);

				drawString(Fonts.font18, "Current APS" + ": §c" + (int) Math.floor((selected.player.aps * 0.8))
						+ " §7- §c" + (int) Math.ceil((selected.player.aps * 2)), padding + 20, infoY + 10, 0xffffffff);

				drawString(Fonts.font18, "Max Yawrate" + ": §c" + (float) selected.maxYawrate, padding + 20, infoY + 20,
						0xffffffff);
