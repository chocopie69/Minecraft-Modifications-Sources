...
public class UnicodeFontRenderer extends FontRenderer {
	...
	public int drawString(String text, double d, double e, int color) {
		return this.drawString(text, (float) d, (float) e, new org.newdawn.slick.Color(color));
	}
	...
	public int drawString(String string, float x, float y, org.newdawn.slick.Color color) {
		if (string == null)
			return 0;
		// glClear(256);
	...
	public int drawStringWithShadow(String string, float x, float y, int color) {
		if(color > 0xffa0a0a0) {
			drawString(string, x + 0.5, y + 0.5, RenderUtil.toRGBA(Color.BLACK));
			drawString(string, (int) x, (int) y, color);
		}
		else {
			drawString(string, x + 0.5, y + 0.5, color);
		}
		return (int) x;
	}
	
	public int drawStringWithShadow(String string, float x, float y, Color color) {
		Color backClr = new Color(0, 0, 0, color.getAlpha());
