...
import summer.cheat.cheats.player.NoHurtcam;
import summer.cheat.cheats.render.ViewClip;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            if (CheatManager.getInstance(NoHurtcam.class).isToggled()) {
                float cameffect = 0.0f;
                f = f / (float) entitylivingbase.maxHurtTime;
                f = MathHelper.sin(f * f * f * f * (float) Math.PI);
                float f2 = entitylivingbase.attackedAtYaw;
                GlStateManager.rotate(-f2, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(-f * cameffect, 0.0F, 0.0F, 1.0F);
                GlStateManager.rotate(f2, 0.0F, 1.0F, 0.0F);
            }
            f = f / (float) entitylivingbase.maxHurtTime;
            f = MathHelper.sin(f * f * f * f * (float) Math.PI);
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (movingobjectposition != null) {
                        ...
                        if (d7 < d3 && (!CheatManager.getInstance(ViewClip.class).isToggled())) {
                            d3 = d7;
                        }
                    }
                }
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.theWorld != null) {
                ...
                this.renderWorld(scaledresolution, p_181560_1_, System.nanoTime() + k);
