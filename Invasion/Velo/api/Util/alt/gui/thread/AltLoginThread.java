...
import Velo.api.Util.alt.gui.thread.AlteningServiceType;
...
extends Thread {
	...
	public static Session createSession(String username, String password) {
		  if(mojang){
			  Main.instance.switchToMojang();;
	        }else {
	        	Main.instance.switchToTheAltening();;
	        }
	...
	public static Session createSession2(String username, String password) {
		Main.instance.switchToMojang();;
      YggdrasilAuthenticationService service = new YggdrasilAuthenticationService(Proxy.NO_PROXY, "");
      YggdrasilUserAuthentication auth = (YggdrasilUserAuthentication)service.createUserAuthentication(Agent.MINECRAFT);
      auth.setUsername(username);
    ...
    public void run() {
        ...
        if (auth == null) {
            ...
            GuiAltManager.currentAccount = new Account(this.username, this.password);
