package me.aidanmees.trivia.gui.custom;
...
public class SearchBar extends Gui {
	...
	public void render(int w, int h, int mouseX, int mouseY) {
		...
		drawHorizontalLine(x, width, height - 15, 0xff000000);
		drawVerticalLine(x, height, height - 15, 0xff000000);
		...
		if (typing) {
			fontRenderer.drawString(typed + "_", x + 5, y + 3, 0xffcfcfcf);
		} else {
			fontRenderer.drawString("Search...", x + 5, y + 3, 0xffcccccc);
		}
	}
	...
	public void keyTyped(char typedChar, int keyCode) {
		typing = true;
		// trivia.chatMessage(Character.toString(typedChar));
		// trivia.chatMessage(keyCode);
		if (keyCode == 1) { // Escape
			typing = false;
			return;
		}
