...
import me.aidanmees.trivia.client.tools.RenderingUtils;
...
public class Button {
    ...
    public void drawCenteredString(String text, float scale) {
        ...
        while ((float)Minecraft.fontRendererObj.getStringWidth(text) * scale > (float)this.getWidth()) {
        ...
        int strWidth = Minecraft.fontRendererObj.getStringWidth(text);
        strWidth = (int)((float)strWidth * scale);
        int x = this.x1 + this.getWidth() / 2 - strWidth / 2;
        int y = this.y1 + this.getHeight() / 2 - 4;
        x = (int)((float)x / scale);
        y = (int)((float)y / scale);
        Minecraft.fontRendererObj.drawStringWithShadow(text, x, y, -1);
