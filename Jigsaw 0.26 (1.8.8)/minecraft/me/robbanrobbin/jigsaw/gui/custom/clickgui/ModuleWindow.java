...
public class ModuleWindow extends Container {
	...
	public void draw() {
		...
		glEnd();
		if(extended) {
			GL11.glColor4f(0.8f, 0.2f, 0.2f, 0.8f - Jigsaw.getClickGuiManager().getAlpha());
			glBegin(GL11.GL_QUADS);
			{
				glVertex2d(-1, 0);
				glVertex2d(-1, getHeight() + 1);
			...
			glBegin(GL11.GL_QUADS);
			{
				...
				glVertex2d(getWidth(), getHeight() + 1);
...
//			GL11.glLineWidth(1);
		...
		if(title != null) {
			titleRenderer.drawString(title, 2, 1, new Color(1f, 1f, 1f, 1f - Jigsaw.getClickGuiManager().getAlpha()));
		}
